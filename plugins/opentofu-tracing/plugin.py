...
- Tracedown: https://github.com/spacelift-solutions/tracedown
    ...
    __contexts__ = [
        Context(
            ...
            hooks={
                "before_init": [
                    "export OTEL_SERVICE_NAME='opentofu-init'"
                ],
                "before_plan": [
                    "export OTEL_SERVICE_NAME='opentofu-plan'"
                ],
                "before_apply": [
                    "export OTEL_SERVICE_NAME='opentofu-apply'"
                ]
            }
        )
    ]
    ...
    def _start_tracedown(self) -> bool:
        ...
        if os.path.exists(self._pid_file):
            try:
                ...
                self.logger.debug("Stale PID file found, will start new tracedown process")
        ...
        try:
            ...
            cmd = [
                ...
                "-grpc-port", self._grpc_port,
                "-http-port", self._http_port,
                "-max-traces", self._max_traces,
                "-max-memory-mb", self._max_memory_mb,
                "-output", output_file,
            ]
            ...
            self.logger.info(f"Tracedown started successfully (PID: {pid}, output: {output_file})")
            self.logger.debug(f"PID saved to {self._pid_file}")
            return True

        except FileNotFoundError:
            self.logger.error(f"Tracedown binary not found: {self._tracedown_binary}")
            self.logger.error("The plugin should have automatically installed tracedown. Please check if the binary installation hook ran successfully.")
            return False
    ...
    def _stop_tracedown(self) -> bool:
        ...
        if not os.path.exists(self._pid_file):
            self.logger.warning("Tracedown PID file not found - tracedown may not be running")
            return False
        ...
        try:
            ...
            self.logger.info(f"Stopping tracedown (PID: {pid}) and generating markdown report...")
            ...
            while elapsed < max_wait:
                ...
                self.logger.error("Tracedown did not shut down gracefully, forcing termination")
                try:
    ...
    def _upload_trace(self, phase_label: str = "Unknown") -> bool:
        ...
        try:
            ...
            try:
                self.logger.info(f"Uploading traces markdown for {phase_label} ({len(markdown_content)} bytes)")
    ...
    def before_init(self) -> None:
        ...
        if not self._start_tracedown():
            self.logger.error("Failed to start tracedown - traces will not be collected")
    ...
    def after_plan(self) -> None:
        ...
        if self._stop_tracedown():
            ...
            self.logger.warning("Failed to stop tracedown after plan - traces may be incomplete")
    ...
    def before_apply(self) -> None:
        ...
        if not self._start_tracedown():
            self.logger.error("Failed to start tracedown - apply traces will not be collected")
    ...
    def after_apply(self) -> None:
        ...
        if self._stop_tracedown():
            ...
            self.logger.warning("Failed to stop tracedown after apply - traces may be incomplete")
